...
public:
	...
	ctree_map_p()
	{
		...
		nvobj::transaction::run(pop, [&] {
			this->root = nvobj::make_persistent_secure<entry>();
		});
	}
	...
	insert(key_type key, value_type value)
	{
		...
		nvobj::transaction::run(pop, [&] {
			if (dest_entry->key == 0 || dest_entry->key == key) {
				nvobj::delete_persistent_secure<T>(dest_entry->value);
				*dest_entry = e;
			} else {
				insert_leaf(
					&e,
	...
	insert_new(key_type key, const Args &... args)
	{
		auto pop = nvobj::pool_by_vptr(this);
		nvobj::transaction::run(pop, [&] {
			return insert(key, nvobj::make_persistent_secure<T>(args...));
		});
		...
		nvobj::transaction::run(pop, [&] {
			if (parent == nullptr) {
				...
				*parent = *(
				...
				nvobj::delete_persistent_secure<entry>(n->entries[0]);
				nvobj::delete_persistent_secure<entry>(n->entries[1]);
				nvobj::delete_persistent_secure<node>(n);
			}
		});
	...
	remove_free(key_type key)
	{
		...
		nvobj::transaction::run(
			pop, [&] { nvobj::delete_persistent_secure<T>(remove(key)); });
		return 0;
	}
	...
	clear()
	{
		...
		nvobj::transaction::run(pop, [&] {
			if (this->root->inode) {
				...
				nvobj::delete_persistent_secure<node>(
					this->root->inode);
				this->root->inode = nullptr;
			}

			nvobj::delete_persistent_secure<T>(this->root->value);
...
private:
	...
	struct entry {
		...
		clear()
		{
			if (inode) {
				inode->clear();
				nvobj::delete_persistent_secure<node>(inode);
				inode = nullptr;
			}
			nvobj::delete_persistent_secure<T>(value);
			value = nullptr;
		}
	};
	...
	struct node {
		...
		clear()
		{
			if (entries[0]) {
				entries[0]->clear();
				nvobj::delete_persistent_secure<entry>(entries[0]);
				entries[0] = nullptr;
			}
			if (entries[1]) {
				entries[1]->clear();
				nvobj::delete_persistent_secure<entry>(entries[1]);
				entries[1] = nullptr;
			}
		}
	};
	...
	{
		auto new_node = nvobj::make_persistent_secure<node>();
		new_node->diff = diff;

		int d = BIT_IS_SET(e->key, new_node->diff);
		new_node->entries[d] = nvobj::make_persistent_secure<entry>(*e);
		...
		new_node->entries[!d] =
			nvobj::make_persistent_secure<entry>(*dest_entry);
		dest_entry->key = 0;
