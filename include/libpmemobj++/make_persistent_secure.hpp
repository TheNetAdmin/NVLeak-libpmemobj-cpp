...
#ifndef LIBPMEMOBJ_CPP_MAKE_PERSISTENT_SECURE_HPP
#define LIBPMEMOBJ_CPP_MAKE_PERSISTENT_SECURE_HPP
...
make_persistent_secure(allocation_flag flag, Args &&... args)
{
	...
	persistent_ptr<T> ptr =
		nvleak::secure_alloc(pmemobj_tx_xalloc, nvleak::page_field{}, sizeof(T), detail::type_num<T>(), flag.value);
...
make_persistent_secure(Args &&... args)
{
	return make_persistent_secure<T>(allocation_flag::none(),
...
template <typename T>
void
delete_persistent_secure(typename detail::pp_if_not_array<T>::type ptr)
{
	if (pmemobj_tx_stage() != TX_STAGE_WORK)
		throw pmem::transaction_scope_error(
...
#endif /* LIBPMEMOBJ_CPP_MAKE_PERSISTENT_SECURE_HPP */
