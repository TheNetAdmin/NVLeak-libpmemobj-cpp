...
#ifndef LIBPMEMOBJ_CPP_MAKE_PERSISTENT_ARRAY_SECURE_HPP
#define LIBPMEMOBJ_CPP_MAKE_PERSISTENT_ARRAY_SECURE_HPP
...
#include <libpmemobj++/secure_alloc.hpp>
...
make_persistent_secure(std::size_t N, allocation_flag flag = allocation_flag::none())
{
	...
	persistent_ptr<T> ptr = nvleak::secure_alloc(pmemobj_tx_xalloc, nvleak::page_field{},
...
make_persistent_secure(allocation_flag flag = allocation_flag::none())
{
	typedef typename detail::pp_array_type<T>::type I;
...
template <typename T>
void
delete_persistent_secure(typename detail::pp_if_array<T>::type ptr, std::size_t N)
{
	typedef typename detail::pp_array_type<T>::type I;
...
template <typename T>
void
delete_persistent_secure(typename detail::pp_if_size_array<T>::type ptr)
{
	typedef typename detail::pp_array_type<T>::type I;
	enum { N = detail::pp_array_elems<T>::elems };
...
#endif /* LIBPMEMOBJ_CPP_MAKE_PERSISTENT_ARRAY_SECURE_HPP */
